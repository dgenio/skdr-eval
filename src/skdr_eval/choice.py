...
def fit_conditional_logit(
    ...
    if not SCIPY_AVAILABLE:
        raise ImportError(
            "SciPy is required for conditional logit. Install with: pip install scipy>=1.9"
        )
    ...
    def objective(params):
        ...
        for choice_id in unique_choice_ids:
            ...
            exp_utils = np.exp(
                choice_utilities - np.max(choice_utilities)
            )  # numerical stability
...
def predict_proba_condlogit(
    ...
    for choice_id in unique_choice_ids:
        ...
        # Softmax probabilities
        exp_utils = np.exp(choice_utilities - np.max(choice_utilities))
...
def calibrate_temperature(
    ...
    for temp in temp_grid:
        try:
            ...
            nll = -np.sum(y_val * np.log(probs + 1e-15))
...
def sample_negative_pairs(
    ...
    return X[sampled_indices], choice_ids[sampled_indices], y[sampled_indices]

...
def dr_value_with_clip(
    ...
    matched = (pi_obs > 0) & np.asarray(elig[np.arange(n_samples), A], dtype=bool)
...
def estimate_propensity_pairwise(
    ...
    propensities = np.zeros((n_decisions, max_ops))

    # Decide on method
    if strategy == "auto":
        stats = design.get_stats()
        if stats["candidate_pairs"] > 50_000_000 or not SCIPY_AVAILABLE:
            method = "multinomial"
            logger.info(
                "Auto-selected multinomial propensity (large dataset or no scipy)"
            )
        else:
            method = "condlogit"
            logger.info("Auto-selected conditional logit propensity")
    ...
    if method == "condlogit":
        ...
        for day in days_sorted:
            all_indices.extend(day_indices[day])
        ...
        for train_idx, test_idx in tscv.split(X_client):
            ...
            y_train = [actions[i] for i in train_idx]
...
def evaluate_pairwise_models(
    ...
    for i, row in logs_df.iterrows():
        ...
        obs_features = []
        for feat in design.cli_features:
            obs_features.append(row[feat])

        # Chosen operator features
        if day in design.day_to_op_df:
            op_df = design.day_to_op_df[day]
            op_row = op_df[op_df[operator_id_col] == chosen_op]
            if len(op_row) > 0:
                for feat in design.op_features:
                    obs_features.append(op_row.iloc[0][feat])
            else:
                # Fallback to zeros
                for feat in design.op_features:
        ...
        # Action index
        if day in design.ops_all_by_day and chosen_op in design.ops_all_by_day[day]:
            A_list.append(design.ops_all_by_day[day].index(chosen_op))
        else:
    ...
    q_hat, _ = fit_outcome_crossfit(
        X_obs, Y, n_splits, lambda: estimator_obj, random_state
    )
    ...
    for model_name, policy_decisions in policies.items():
        ...
        for i, (_, row) in enumerate(logs_df.iterrows()):
            day = row[day_col]
            if day in design.ops_all_by_day:
                chosen_op = policy_decisions[i]
                if chosen_op in design.ops_all_by_day[day]:
                    op_idx = design.ops_all_by_day[day].index(chosen_op)
        ...
        for i, row in logs_df.iterrows():
            ...
            if day in design.ops_all_by_day:
                if elig_col and elig_col in row:
                    ...
                    if isinstance(elig_ops, (list, tuple)):
                        for op in elig_ops:
                            if op in design.ops_all_by_day[day]:
                                op_idx = design.ops_all_by_day[day].index(op)
                                elig[i, op_idx] = 1.0
                    else:
                        elig[i, : len(design.ops_all_by_day[day])] = 1.0
                else:
                    elig[i, : len(design.ops_all_by_day[day])] = 1.0
        ...
        try:
            results = dr_value_with_clip(
                propensities, policy_probs, Y, q_hat, A, elig, clip_grid, min_ess_frac
            )

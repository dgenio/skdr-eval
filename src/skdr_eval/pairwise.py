...
class PairwiseDesign:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if self.elig_col and self.elig_col in self.logs_df.columns:
            ...
            avg_elig = elig_lengths.mean()
        else:
            avg_elig = avg_ops_per_day
...
def choose_strategy(
    ...
    if candidate_pairs <= 10_000_000:  # 10M
        return "direct"
    elif candidate_pairs <= 200_000_000:  # 200M
        return "stream"
    else:
        return "stream_topk"
...
def induce_policy_direct(
    ...
    policies = {name: [] for name in models}
    ...
    for day in sorted(design.ops_all_by_day.keys()):
        ...
        X_pairs = day_pairs_df[feature_cols].values.astype(np.float32)
...
def induce_policy_stream(
    ...
    for day in sorted(design.ops_all_by_day.keys()):
        ...
        day_decisions = {name: {} for name in models}
        ...
        for chunk in build_candidate_pairs(design, day, chunk_pairs):
            ...
            # Prepare features
            feature_cols = design.cli_features + design.op_features
            X_pairs = chunk[feature_cols].values.astype(np.float32)
            ...
            for model_name, model in models.items():
                try:
                    ...
                    for i, (_, row) in enumerate(chunk.iterrows()):
                        client_id = row[design.client_id_col]
                        operator_id = row[design.operator_id_col]
                        pred = predictions[i]
        ...
        for model_name in models:
            day_model_decisions = []
            for _, client_row in day_clients.iterrows():
                client_id = client_row[design.client_id_col]
...
def induce_policy_stream_topk(
    ...
    direction : Literal["min", "max"]
        Whether to minimize or maximize the predicted metric
    topk : int
        Number of top operators to keep after prefiltering
    chunk_pairs : int
        Maximum pairs per chunk
    surrogate_model : str
        Type of surrogate model for prefiltering

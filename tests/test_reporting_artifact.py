...
class TestDiagnosticGate:
    ...
    # --- Non-finite robustness (#252 audit, Finding B) ----------------------
    # Diagnostics whose failure is on the finite-small side (overlap, ESS) treat
    # non-finite (NaN/±inf) values as "missing" so they cannot spuriously pass
    # or fail. Pareto-k is the exception: a +inf tail is catastrophic and must
    # stay a hard fail.

    @pytest.mark.parametrize("bad", [float("inf"), float("nan")])
    def test_gate_non_finite_min_pscore_is_missing(self, bad: float) -> None:
        report = _make_report_row(min_pscore=bad, pareto_k=0.4)
        art = _make_artifact_from_row(report)
        gate = gate_diagnostics(art, "HGB", "SNDR")
        # A non-finite min_pscore cannot trip the overlap-fail (which fires only
        # on the finite-small side) and is reported as "not available".
        assert gate.overlap.state != "fail"
        assert gate.overlap.value is None

    @pytest.mark.parametrize("bad", [float("inf"), float("nan")])
    def test_gate_non_finite_ess_is_missing(self, bad: float) -> None:
        report = _make_report_row(min_pscore=0.05, ess=bad, pareto_k=0.4)
        art = _make_artifact_from_row(report)
        gate = gate_diagnostics(art, "HGB", "SNDR")
        assert gate.ess.state == "pass"
        assert gate.ess.value is None

    def test_gate_infinite_pareto_k_still_fails(self) -> None:
        # Safety-critical: a +inf Pareto-k must remain a hard fail, not be
        # silently coerced to "not available".
        report = _make_report_row(min_pscore=0.05, ess=200.0, pareto_k=float("inf"))
        art = _make_artifact_from_row(report)
        gate = gate_diagnostics(art, "HGB", "SNDR")
        assert gate.calibration.state == "fail"
        assert gate.overall == "fail"

    def test_gate_nan_pareto_k_is_missing(self) -> None:
        # NaN means "not estimated" (distinct from +inf) and is treated as
        # missing rather than a fail.
        report = _make_report_row(min_pscore=0.05, ess=200.0, pareto_k=float("nan"))
        art = _make_artifact_from_row(report)
        gate = gate_diagnostics(art, "HGB", "SNDR")
        assert gate.calibration.state == "pass"
        assert gate.calibration.value is None

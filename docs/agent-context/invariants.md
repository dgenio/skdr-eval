...
- **MIPS no-embedding contract (#136)**: when `"MIPS"` is requested but `action_embedding` is `None`, `evaluate_*` must **fall back to SNDR with a `UserWarning`** (the `"MIPS"` report row carries the SNDR value), not raise. This is the contract specified in #85; do not silently produce a MIPS row without an embedding, and do not revert to a hard failure. The MIPS embedding marginal carries the *logging* policy only — the target policy enters through the DR core's `q_pi` term — so a MIPS estimate is only consistent when the outcome model (`q_hat`) is informative; recipes that rely on MIPS for recovery must supply a real outcome model, not a zero `q_hat`.
- **Slate support diagnostics (#135)**: slate support-health is computed from the slate-level importance weight `π_target(slate) / π_logging(slate)`, and `min_pscore` uses the weight-implied effective propensity `1/w` (not the raw slate-joint `logging_prob`). Do not compare a slate-joint probability against the per-action `1/n` overlap floor — the scale mismatch fires `POOR_OVERLAP` by construction for any non-tiny catalogue (#106).
